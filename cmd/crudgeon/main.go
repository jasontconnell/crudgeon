...
func main() {
	...
	if *dir == "" {
		pfile, err := process.ParseFile(*file, basetypeMap, cfg.NullableFormat, cfg.Null, cfg.DbNull, cfg.GenericReg, cfg.NullableReg)
		if err != nil {
		...
		for _, p := range paths {
			pfile, err := process.ParseFile(p, basetypeMap, cfg.NullableFormat, cfg.Null, cfg.DbNull, cfg.GenericReg, cfg.NullableReg)
			if err != nil {
	...
	for _, pfile := range pfiles {
		for _, g := range cfg.Generations {
			...
			gp, err := process.GetGenPackage(*obj, *path, pfile.Fields, g.Database, tmpfile, *ns, g.OutputPrefix, g.OutputSuffix, g.Folder, g.Extension, g.Flags, cfg.ConcreteCollection, cfg.AbstractCollection, pfile.GenFlags, *fld, g.ConditionFlag)
